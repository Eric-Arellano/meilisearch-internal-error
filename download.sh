...
# https://github.com/meilisearch/meilisearch/blob/7e314664fcdb139600bfbb8809462b56cbd41a17/download-latest.sh
# but instead of download the latest version, accepts a version as environment variable
...
download_binary() {
    ...
    echo "Downloading Meilisearch binary $version for $os, architecture $archi..."
    case "$os" in
        'windows')
    ...
    curl --fail -OL "$GITHUB_REL/$version/$release_file"
    if [ $? -ne 0 ]; then
        fetch_release_failure_usage
        exit 1
    fi
